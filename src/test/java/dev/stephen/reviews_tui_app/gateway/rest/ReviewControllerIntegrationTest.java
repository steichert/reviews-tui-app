...
class ReviewControllerIntegrationTest {
  ...
  void searchColleagues_substringMatch_returnsMatch() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/colleagues/search").param("q", "alice"))
        .andExpect(status().isOk())
  ...
  void searchColleagues_fuzzyMatch_returnsMatch() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/colleagues/search").param("q", "Alise"))
        .andExpect(status().isOk())
  ...
  void searchColleagues_noMatch_returnsEmptyArray() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/colleagues/search").param("q", "zzz"))
        .andExpect(status().isOk())
        .andExpect(content().json("[]"));
  }
  ...
  void getSummaryForColleague_returnsCorrectCounts() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/notes/summary").param("colleague", "Alice Smith"))
        .andExpect(status().isOk())
  ...
  void getSummaryForAll_groupsNotesByColleague() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/notes/summary").param("from", today).param("to", today))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.length()").value(2));
  }
  ...
  void getSummary_noParams_returns400() throws Exception {
    mockMvc.perform(get("/api/v1/notes/summary")).andExpect(status().isBadRequest());
  }
}
