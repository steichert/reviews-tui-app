...
class ColleagueServiceImplTest {
    ...
    void setUp() {
        ...
        alice = Colleague.builder().id(1L).name("Alice Smith").createdAt(now).updatedAt(now).build();
    ...
    void listColleagues_returnsAllMappedDtos() {
        ...
        assertThat(result).extracting(ColleagueDto::name).containsExactly("Alice Smith", "Bob Jones");
    ...
    void findByNameFuzzy_substringMatch_returnsSubstringResults() {
        when(colleagueRepository.findByNameContainingIgnoreCase("alice")).thenReturn(List.of(alice));
    ...
    void findByNameFuzzy_noSubstringMatch_sortsResultsByScoreDescending() {
        ...
        Colleague alise = Colleague.builder().id(3L).name("Alise Smith")
