...
public class ColleagueServiceImpl implements ColleagueService {
    ...
    public List<ColleagueDto> findByNameFuzzy(String query) {
        List<Colleague> substringMatches =
                colleagueRepository.findByNameContainingIgnoreCase(query);
        ...
        return colleagueRepository.findAll().stream()
                .filter(
                        c ->
                                jaroWinklerSimilarity.apply(c.getName().toLowerCase(), lowerQuery)
                                        >= threshold)
                .sorted(
                        (a, b) ->
                                Double.compare(
                                        jaroWinklerSimilarity.apply(
                                                b.getName().toLowerCase(), lowerQuery),
                                        jaroWinklerSimilarity.apply(
                                                a.getName().toLowerCase(), lowerQuery)))
                .map(this::toDto)
                .toList();
    }
